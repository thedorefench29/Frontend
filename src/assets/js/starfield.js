(function(root, factory) {
  ...
  function setup(userConfig = {}) {
    ...
    if (config.auto) {
      origin = config.originElement || document.querySelector(".cta-button");
      if (!origin) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const origin = document.querySelector(".cta-button");

    if (!origin) {
      console.error("Button with class .cta-button not found.");
      return;
    }
